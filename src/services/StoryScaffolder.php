...
class StoryScaffolder extends Component
{
    ...
    public function render(string $title, ?string $description, array $stories, ?string $warning = null): string
    {
        return sprintf(
             ...
             *
             * 'group' is empty so the component keeps the group it inherits from
             * its folder or marker file. Fill it in to override that.
             */
    ...
    {
        // Every meta key the parser understands, in its order — so a generated
        // file doubles as an honest reference for the vocabulary instead of
        // implying that the three keys it happens to fill are all there is.
        //
        // `group` goes out EMPTY on purpose, and that is not a placeholder:
        // StoryParser::normalizeMeta() trims and drops empty values, so an empty
        // group is byte-for-byte the same as no group at all, and the component
        // keeps inheriting from the folder hierarchy or the marker file above
        // it. Writing a real group into every scaffold would freeze that
        // inheritance — renaming a marker's H1 would stop moving anything.
