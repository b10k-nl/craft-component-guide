...
1. **Режим «показувати недокументовані»** *(був у беклозі — піднято пріоритет).*
   Сканер показує всі Twig-файли кореня, навіть без story: бейдж «undocumented»,
   без прев'ю або з рендером без аргументів. Нуль зусиль — і гід уже цінний
   як інвентаризація компонентів.
...
- Межа безкоштовне-проти-платне (що лишається у безкоштовній редакції Lite, якщо взагалі).
  Зокрема: персистентний кеш уже реалізовано в ядрі (без нього Lite занадто
  повільний на великих проєктах) — рядок «Персистентний кеш» у таблиці §6
  переглянути під час фіналізації едишенів.
